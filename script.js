...
////////// INICIO DA FUNÇÃO CLIQUE
  ...
  function clique(event) {
     ...
     const {positions, positionsBombas, quantidadeCliques, spanEBombasRedor, posicoesValidas} = dados;
...
const seForBandeira = filho ? filho.getAttribute("modo") === "bandeira" : false;
...
function onlongClick({currentTarget, target}) {
  ...
  const filho = currentTarget.firstChild ? currentTarget.firstChild : undefined
  const modo = filho ? filho.getAttribute("modo") : undefined
  
  const seInserirBandeira = 
  modo === "bomba" &&
  Boolean(filho.getAttribute("explodiu")) === false;
  
  const seVazio = Boolean(div.getAttribute("vazio"));
  
  const seJaAtivo = Boolean(div.getAttribute("ativo"))
  
  console.log(seJaAtivo)
...
////////// INICIO FUNÇÃO Função para definir a posição das bombas
  ...
  function bombasPosition(emVoltaClique) {
    ...
    while (dados.positionsBombas.length < quantidadeBombas) {
      ...
      numeroGerado) && !emVoltaClique.includes(numeroGerado)) positionsBombas.push(numeroGerado);
...
function indexEmVolta(index) {
    ...
    const positionsValidas = positions.filter(item => item >= 1 && item <= max)
